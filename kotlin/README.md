...
Версия: **2.6.0**. Полный справочник публичного API: [API.md](API.md).
...
> CA для задач отладки может проигнорировать запрошенный EKU. После enroll всегда валидируйте leaf.
...
На iOS выполняется **тот же набор unit-тестов**, что и на JVM. Тесты в **`commonTest`**; платформо-специфичным остаётся только чтение файловых фикстур.
...
**Проверка опубликованного Maven-артефакта** (как внешний потребитель): временно замените зависимость на `com.atom:sgw-registry:2.6.0`, опубликуйте в `kotlin/dist/maven/` и запустите smoke-тест (см. [API.md — Проверка опубликованного пакета](API.md#проверка-опубликованного-пакета-тесты-и-примеры)).
...
Аналогичная структура разделов: [API.md — JVM-примеры](API.md#jvm-примеры-registry-examples).
...
**v2.6.0:** CloudConfigFromContext (invitation → TBOX), CloudBrokerFqdn (CES §5), sign-tbox / empty-owner examples; CloudConfigCms + KMP jvm / android / ios.  
